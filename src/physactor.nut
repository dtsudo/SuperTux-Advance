...
PhysAct <- class extends Actor {
		...
		foreach (i in actor["MoPlat"]) {
			if (hitTest(shape, i.shape)) {
				...
				for (local j = 0.0; j < 1000; j += 0.1) {
					...
					// Down
					shape.setPos(x, y + j);
					if (!hitTest(shape, i.shape) && placeFree(x, y + j)) {
						result = 1;
						y += j;
						break;
					}
					...
					if (!hitTest(shape, i.shape) && placeFree(x, y - j)) {
						result = -1;
						y -= j;
						if (!hitTest(shape, i.shape) && i.vspeed > 0)
							y += i.vspeed;
						break;
					}
				}
	...
	function placeFree(_x, _y, s = null) {
			...
			for (local i = -cw; i <= cw; i++) {
				for (local j = -ch; j <= ch; j++) {
					...
					if (tile >= 0 && tile < wl.data.len())
						switch (wl.data[tile] - gvMap.solidfid) {
							case 0: // Full solid
							case 39:
							case 40:
							case 42:
							...
							case 82:
								...
								if (typeof shape == "Cir")
									nps = Cir(shape.x + shape.ox, ns.y, ns.r);
								gvMap.shape.setPos(
									(cx + i) * 16 + 8,
									(cy + j) * 16 + 4
								);
								...
								gvMap.shape.w = 8.0;
								gvMap.shape.h = 4.0;
								if (nps.y >= shape.y + shape.oy || vspeed > 0)
									if (
										hitTest(nps, gvMap.shape) &&
										!hitTest(shape, gvMap.shape) &&
										hitTest(ns, gvMap.shape)
									)
										return false;
								break;
		...
		for (local i = 0; i < gvMap.geo.len(); i++) {
			if (gvMap.geo[i] != null)
				if (hitTest(ns, gvMap.geo[i])) return false;
		}

		return true;
	}

	function escapeSolid(ts = null) {
		// Save current location and move
		local ns;
		if (typeof shape == "Rec")
			ns = Rec(x + shape.ox, y + shape.oy, shape.w, shape.h, shape.kind);
		if (typeof shape == "Cir")
			ns = Cir(x + shape.ox, y + shape.oy, shape.r);
		local cx = floor(x / 16);
		local cy = floor(y / 16);
		...
		for (local i = 0; i < 1000; i++) {
			if (placeFree(x + i, y) && (ts != null ? !hitTest(ns, ts) : true)) {
				x += i;
				break;
			}
			if (placeFree(x - i, y) && (ts != null ? !hitTest(ns, ts) : true)) {
			...
			if (placeFree(x, y + i) && (ts != null ? !hitTest(ns, ts) : true)) {
				y += i;
				break;
			}
			if (placeFree(x, y - i) && (ts != null ? !hitTest(ns, ts) : true)) {
		...
		if (wl != null) {
			for (local i = -cw; i <= cw; i++) {
				for (local j = -ch; j <= ch; j++) {
					local tile = cx + i + (cy + j) * wl.width;
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 37) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 8
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 40) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 8
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 42) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 8
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 43) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 8
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 60) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 4
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 61) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 12
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 62) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 4,
								(cy + j) * 16 + 8
							);
					...
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 63) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 12,
								(cy + j) * 16 + 8
							);
		...
		if (wl != null) {
			for (local i = -cw; i <= cw; i++) {
				for (local j = -ch; j <= ch; j++) {
					local tile = cx + i + (cy + j) * wl.width;
					if (tile >= 0 && tile < wl.data.len())
						if (wl.data[tile] - gvMap.solidfid == 41) {
							gvMap.shape.setPos(
								(cx + i) * 16 + 8,
								(cy + j) * 16 + 8
							);
	...
	function onPlatform(_x = 0, _y = 0) {
		...
		if (typeof shape == "Cir")
			ns = Cir(x + shape.ox + _x, y + shape.oy + 2 + _y, shape.r);
		local cx = floor(x / 16);
	...
	function onIce(_x = -1, _y = -1) {
		...
		if (typeof shape == "Cir")
			ns = Cir(x + shape.ox, y + shape.oy + 2, shape.r);
		local cx = floor(x / 16);
		local cy = floor(y / 16) + 1;
		...
		if (wl != null) {
			...
			if (tile >= 0 && tile < wl.data.len())
				switch (wl.data[tile] - gvMap.solidfid) {
					...
					case 101:
						if (typeof shape == "Rec")
							ns = Rec(x, y, shape.w, shape.h, shape.kind);
	...
	function handleConveyor() {
		...
		local pl = tileGetSolid(
			x + shape.ox - shape.w + hspeed,
			y + shape.oy + shape.h + 1
		);
		local pr = tileGetSolid(
			x + shape.ox + shape.w + hspeed,
			y + shape.oy + shape.h + 1
		);
		...
		if ([pc, pl, pr].find(104) != null && placeFree(x - cs, y)) x += cs;

		if ([pc, pl, pr].find(103) != null && placeFree(x + cs, y)) x -= cs;
	}
};
