...
drawMeter <- function (x, y, m, v, c) {
	...
	for (local i = 0; i < m; i++) drawSprite(sprMeterBack, 1, x + i * 2 + 2, y);
...
startPlay <- function (level, newLevel = true, skipIntro = false) {
	...
	if (gvTimeAttack && gvTARandomPlayer) {
		local cl = [];
		foreach (key, i in gvCharacters) cl.push(key);
	...
	foreach (i in actlayer.objects) {
		...
		if (i.rawin("gid")) {
			...
			if (mapActor[i.id] in actor)
				switch (typeof actor[mapActor[i.id]]) {
					case "WoodBlock":
					case "BrickBlock":
					case "Coin":
					case "Coin5":
					...
					case "Herring":
						...
						break;
					case "ItemBlock":
						if (actor[mapActor[i.id]].item == 0) gvYetFoundItems[i.id] <- actor[mapActor[i.id]].id;
						break;
				}
		}

		// Rectangle actors
		if (!i.rawin("polygon") && !i.rawin("polyline") && !i.rawin("ellipse") && !i.rawin("point") && !i.rawin("gid"))
			if (i.name != "") {
				local c = 0;
				...
				if (getroottable().rawin(n))
					if (typeof getroottable()[n] == "class") {
						print(i.x + " - " + i.y);
		...
		if (i.rawin("polyline"))
			if (i.name != "") {
				...
				if (getroottable().rawin(n)) {
					// Create polygon to pass to object
					local poly = [];
					for (local j = 0; j < i.polyline.len(); j++)
						poly.push([i.x + i.polyline[j].x, i.y + i.polyline[j].y]);
	...
	if (game.hasPidgin) {
		if ("AttackPidgin" in actor) {
			local sulphurList = [];
			foreach (i in actor["AttackPidgin"]) sulphurList.push(i.id);
			for (local j = 0; j < sulphurList.len(); j++) deleteActor(sulphurList[j]);
		}
	...
	if (game.check)
		foreach (k, i in gvFoundItems) {
			if (k in mapActor && mapActor[k] in actor) {
				print(typeof actor[mapActor[k]]);
				switch (i) {
					case "ItemBlock":
						if (actor[mapActor[k]].item != 0) break;
			...
			for (local j = 0; j < gvMap.data.layers[i].objects.len(); j++) {
				...
				switch (lana) {
					case "secret":
						...
						if ("polyline" in obj)
							for (local j = 0; j < obj.polyline.len(); j++)
								poly.push([obj.x + obj.polyline[j].x, obj.y + obj.polyline[j].y]);
						else
							for (local j = 0; j < obj.polygon.len(); j++)
								poly.push([obj.x + obj.polygon[j].x, obj.y + obj.polygon[j].y]);
...
gmLevelStart <- function () {
	...
	if (gvLangObj["level"].rawin(gvMap.name))
		drawText(font2, gvScreenW / 2 - gvLangObj["level"][gvMap.name].len() * 4, 8, gvLangObj["level"][gvMap.name]);
	...
	if (game.playerChar2 != 0 && game.playerChar2 != "") {
		...
		if (!config.showTF)
			drawSprite(
				getroottable()[gvCharacters[game.playerChar2]["normal"]],
				runAnim[(getFrames() / 4) % runAnim.len()],
				gvScreenW / 2 - charx,
				gvScreenH / 2
			);
		else
			switch (game.ps2.weapon) {
				...
				case "fire":
					drawSprite(
						getroottable()[gvCharacters[game.playerChar2]["fire"]],
						runAnim[(getFrames() / 4) % runAnim.len()],
						gvScreenW / 2 - charx,
						gvScreenH / 2
					);
				...
				case "ice":
					drawSprite(
						getroottable()[gvCharacters[game.playerChar2]["ice"]],
						runAnim[(getFrames() / 4) % runAnim.len()],
						gvScreenW / 2 - charx,
						gvScreenH / 2
					);
				...
				case "air":
					drawSprite(
						getroottable()[gvCharacters[game.playerChar2]["air"]],
						runAnim[(getFrames() / 4) % runAnim.len()],
						gvScreenW / 2 - charx,
						gvScreenH / 2
					);
				...
				case "earth":
					drawSprite(
						getroottable()[gvCharacters[game.playerChar2]["earth"]],
						runAnim[(getFrames() / 4) % runAnim.len()],
						gvScreenW / 2 - charx,
						gvScreenH / 2
					);
				...
				case "shock":
					drawSprite(
						getroottable()[gvCharacters[game.playerChar2]["shock"]],
						runAnim[(getFrames() / 4) % runAnim.len()],
						gvScreenW / 2 - charx,
						gvScreenH / 2
					);
				...
				case "water":
					drawSprite(
						getroottable()[gvCharacters[game.playerChar2]["water"]],
						runAnim[(getFrames() / 4) % runAnim.len()],
						gvScreenW / 2 - charx,
						gvScreenH / 2
					);
	...
	if (!config.showTF)
		drawSprite(
			getroottable()[gvCharacters[game.playerChar]["normal"]],
			runAnim[(getFrames() / 4) % runAnim.len()],
			charx + gvScreenW / 2,
			gvScreenH / 2
		);
		...
		switch (game.ps.weapon) {
			...
			case "fire":
				drawSprite(
					getroottable()[gvCharacters[game.playerChar]["fire"]],
					runAnim[(getFrames() / 4) % runAnim.len()],
					charx + gvScreenW / 2,
					gvScreenH / 2
				);
			...
			case "ice":
				drawSprite(
					getroottable()[gvCharacters[game.playerChar]["ice"]],
					runAnim[(getFrames() / 4) % runAnim.len()],
					charx + gvScreenW / 2,
					gvScreenH / 2
				);
			...
			case "air":
				drawSprite(
					getroottable()[gvCharacters[game.playerChar]["air"]],
					runAnim[(getFrames() / 4) % runAnim.len()],
					charx + gvScreenW / 2,
					gvScreenH / 2
				);
			...
			case "earth":
				drawSprite(
					getroottable()[gvCharacters[game.playerChar]["earth"]],
					runAnim[(getFrames() / 4) % runAnim.len()],
					charx + gvScreenW / 2,
					gvScreenH / 2
				);
			...
			case "shock":
				drawSprite(
					getroottable()[gvCharacters[game.playerChar]["shock"]],
					runAnim[(getFrames() / 4) % runAnim.len()],
					charx + gvScreenW / 2,
					gvScreenH / 2
				);
			...
			case "water":
				drawSprite(
					getroottable()[gvCharacters[game.playerChar]["water"]],
					runAnim[(getFrames() / 4) % runAnim.len()],
					charx + gvScreenW / 2,
					gvScreenH / 2
				);
	...
	if (game.hasPidgin) {
		if (game.playerChar2 != 0 && game.playerChar2 != "")
			drawSprite(
				sprAttackPidgin,
				AttackPidgin.an["fly"][wrap(getFrames() / 4, 0, 7)],
				(game.hasPidgin == 1 ? charx : -charx) + gvScreenW / 2,
				gvScreenH / 2 - 32
			);
		else
			drawSprite(
	...
	if (game.bestTime.rawin(gvMap.name + "-" + game.playerChar))
		bt += formatTime(game.bestTime[gvMap.name + "-" + game.playerChar]);
	else bt += "0:00.00";
...
gmPlay <- function () {
	...
	if (actor.rawin("SecretJoiner"))
		foreach (i in actor["SecretJoiner"]) {
			i.draw();
		}
	if (debug)
		gvMap.drawTiles(floor(-camx), floor(-camy), camx, camy, gvScreenW / 16 + 5, gvScreenH / 16 + 2, "solid", 0.5);
	...
	if (
		(gvPlayer &&
			gvPlayer2 &&
			"invincible" in gvPlayer &&
			"invincible" in gvPlayer2 &&
			gvPlayer.invincible == 0 &&
			gvPlayer2.invincible == 0) ||
		(gvPlayer && !gvPlayer2 && "invincible" in gvPlayer && gvPlayer.invincible == 0) ||
		(gvPlayer2 && !gvPlayer && "invincible" in gvPlayer2 && gvPlayer2.invincible == 0)
	)
		songPlay(gvMusicName);
	...
	if (gvInfoBox == "") {
			...
			if (!gvSplitScreen) {
				if (gvPlayer)
					drawFloatingStats(
						gvPlayer.x - camx0,
						gvPlayer.y - camy0,
						(1.0 / game.maxHealth) * gvBarStats.health1,
						(1.0 / game.ps.maxStamina) * gvBarStats.stamina1,
						(1.0 / game.ps.maxEnergy) * gvBarStats.mana1
					);
				...
				if (gvPlayer2)
					drawFloatingStats(
						gvPlayer2.x - camx0,
						gvPlayer2.y - camy0,
						(1.0 / game.maxHealth) * gvBarStats.health2,
						(1.0 / game.ps2.maxStamina) * gvBarStats.stamina2,
						(1.0 / game.ps2.maxEnergy) * gvBarStats.mana2
					);
			} else {
				if (gvSwapScreen) {
					if (gvPlayer)
						drawFloatingStats(
							gvPlayer.x - camx1 + gvScreenW / 2,
							gvPlayer.y - camy1,
							(1.0 / game.maxHealth) * game.ps.health,
							(1.0 / game.ps.maxStamina) * game.ps.stamina,
							(1.0 / game.ps.maxEnergy) * game.ps.energy
						);
					...
					if (gvPlayer2)
						drawFloatingStats(
							gvPlayer2.x - camx2,
							gvPlayer2.y - camy2,
							(1.0 / game.maxHealth) * game.ps2.health,
							(1.0 / game.ps2.maxStamina) * game.ps2.stamina,
							(1.0 / game.ps2.maxEnergy) * game.ps2.energy
						);
				} else {
					if (gvPlayer)
						drawFloatingStats(
			...
			if (!gvSplitScreen) {
				if (gvPlayer) {
					...
					foreach (i in actor["Herring"]) {
						if (
							nearestFish == null ||
							distance2(i.x, i.y, gvPlayer.x, gvPlayer.y) <
								distance2(nearestFish.x, nearestFish.y, gvPlayer.x, gvPlayer.y)
						)
							nearestFish = i;
					}
					...
					drawSprite(
						...
						gvPlayer.x - camx0,
						gvPlayer.y - camy0,
						pointAngle(gvPlayer.x, gvPlayer.y, nearestFish.x, nearestFish.y)
					);
				}
				...
				if (gvPlayer2) {
					...
					foreach (i in actor["Herring"]) {
						if (
							nearestFish == null ||
							distance2(i.x, i.y, gvPlayer2.x, gvPlayer2.y) <
								distance2(nearestFish.x, nearestFish.y, gvPlayer2.x, gvPlayer2.y)
						)
							nearestFish = i;
					}
					...
					drawSprite(
						...
						gvPlayer2.x - camx0,
						gvPlayer2.y - camy0,
						pointAngle(gvPlayer2.x, gvPlayer2.y, nearestFish.x, nearestFish.y)
					);
				}
			} else {
				if (gvSwapScreen) {
		...
		if (gvBattleMode && gvSplitScreen && gvPlayer && gvPlayer2) {
			...
			if (gvSwapScreen) {
				drawSprite(
					...
					gvPlayer.x - camx1 + gvScreenW / 2,
					gvPlayer.y - camy1,
					pointAngle(gvPlayer.x, gvPlayer.y, target2.x, target2.y)
				);
				drawSprite(
					sprHerrow,
					getFrames() / 16,
					gvPlayer2.x - camx2,
					gvPlayer2.y - camy2,
					pointAngle(gvPlayer2.x, gvPlayer2.y, target1.x, target1.y)
				);
			} else {
				drawSprite(
					sprHerrow,
		...
		if (gvSplitScreen) {
			...
			drawMeter(
				24 + (gvSwapScreen ? gvScreenW / 2 : 0),
				16,
				game.ps.maxEnergy * 2.0,
				gvBarStats.mana1 * 2.0,
				0x1080b0ff
			);
			drawMeter(
				24 + (gvSwapScreen ? gvScreenW / 2 : 0),
				24,
				game.ps.maxStamina * 2.0,
				gvBarStats.stamina1 * 2.0,
				0x70a048ff
			);
			switch (game.ps.weapon) {
				case "normal":
			...
			drawMeter(
				24 + (gvSwapScreen ? 0 : gvScreenW / 2),
				16,
				game.ps.maxEnergy * 2.0,
				gvBarStats.mana2 * 2.0,
				0x1080b0ff
			);
			drawMeter(
				24 + (gvSwapScreen ? 0 : gvScreenW / 2),
		...
		if (game.maxRedCoins > 0)
			drawText(
				font2,
				24,
				gvScreenH - (config.completion ? 72 : 38),
				game.redCoins.tostring() + "/" + game.maxRedCoins.tostring()
			);
		// Draw subitem
		if (gvSplitScreen) {
		...
		if (gvDoIGT && (config.showleveligt || gvTimeAttack) && levelEndRunner != 1)
			drawText(font2, 8, 32 + timey, formatTime(gvIGT));

		// Draw offscreen player
		if (gvPlayer && gvPlayer.y < -8) {
			if ((typeof gvPlayer) in gvCharacters)
				drawSprite(
					getroottable()[gvCharacters[typeof gvPlayer]["doll"]],
					enWeapons[config.showTF ? game.ps.weapon : "normal"],
					gvPlayer.x - camx,
					8 - gvPlayer.y / 4
				);
		}
		...
		if (gvPlayer && "stats" in gvPlayer && gvPlayer.stats.canres) {
			if ((typeof gvPlayer) in gvCharacters)
				drawSprite(
					getroottable()[gvCharacters[typeof gvPlayer]["doll"]],
					enWeapons[config.showTF ? gvPlayer.stats.weapon : "normal"],
					gvScreenW - kx,
					gvScreenH - 16
				);
			kx += 16;
		...
		if (gvPlayer2 && "stats" in gvPlayer2 && gvPlayer2.stats.canres) {
			if ((typeof gvPlayer2) in gvCharacters)
				drawSprite(
					getroottable()[gvCharacters[typeof gvPlayer2]["doll"]],
					enWeapons[config.showTF ? gvPlayer2.stats.weapon : "normal"],
					gvScreenW - kx,
					gvScreenH - 16
				);
			kx += 16;
	...
	if (game.ps.berries > 0 && game.ps.berries >= 12 && game.ps.health > 0) {
		if (game.ps.health < game.maxHealth) {
			game.ps.health++;
			game.ps.berries = 0;
			if (gvPlayer) newActor(Heal, gvPlayer.x - 16 + randInt(32), gvPlayer.y - 16 + randInt(32));
	...
	if (game.ps2.berries > 0 && game.ps2.berries >= 12 && game.ps2.health > 0) {
		if (game.ps2.health < game.maxHealth) {
			game.ps2.health++;
			game.ps2.berries = 0;
			if (gvPlayer2) newActor(Heal, gvPlayer2.x - 16 + randInt(32), gvPlayer2.y - 16 + randInt(32));
...
playerTeleport <- function (target = false, _x = 0, _y = 0) {
	// Used to move the player and camera at the same time
	if (!target) return;
	if (gvMap == 0) return;
...
createPlatformActors <- function (n, i, c) {
	switch (n) {
			...
			if (gvPlayAsBeam) {
				...
				if (gvTARandomPlayer && !gvTimeAttack) {
					...
					foreach (k, i in gvCharacters) {
						if (k in game.characters) pl.push(k);
					}
		...
		case 12:
			if (game.difficulty >= 3) c = newActor(CaptainMorel, i.x + 8, i.y - 8, i.name);
			else c = newActor(Deathcap, i.x + 8, i.y - 8, false);
			game.maxEnemies++;
			break;
		...
		case 13:
			if (game.difficulty >= 3) {
				...
				actor[c].moving = true;
			} else c = newActor(Deathcap, i.x + 8, i.y - 8, true);
			game.maxEnemies++;
			break;
		...
		case 23:
			if (gvLangObj["devcom"].rawin(i.name))
				c = newActor(KelvinScarf, i.x + 8, i.y - 8, gvLangObj["devcom"][i.name]);
		...
		case 26:
			if (game.difficulty >= 3) c = newActor(Haywire, i.x + 8, i.y - 8, i.name);
			else c = newActor(CarlBoom, i.x + 8, i.y - 8);
			game.maxEnemies++;
			break;
		...
		case 28:
			if (game.difficulty >= 3) c = newActor(RedFish, i.x + 8, i.y - 8, i.name);
			else c = newActor(BlueFish, i.x + 8, i.y - 8);
			game.maxEnemies++;
			break;

		case 29:
			if (game.difficulty >= 3) c = newActor(GreenFish, i.x + 8, i.y - 8, i.name);
			else c = newActor(RedFish, i.x + 8, i.y - 8);
			game.maxEnemies++;
			break;
		...
		case 44:
			if (game.difficulty >= 3) c = newActor(Puffranah, i.x + 8, i.y - 8, i.name);
			else c = newActor(GreenFish, i.x + 8, i.y - 8);
			game.maxEnemies++;
			break;
		...
		case 64: // Custom actor gear
			...
			if (getroottable().rawin(n))
				if (typeof getroottable()[n] == "class") c = newActor(getroottable()[n], i.x + 8, i.y - 8, arg);
			break;
		...
		case 78:
			if (game.difficulty >= 3) c = newActor(Poof, -100, -100);
			else c = newActor(Berry, i.x + 8, i.y - 8);
			break;
