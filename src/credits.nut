...
startCredits <- function (folder = "res") {
	...
	for (local i = 0; i < creditsData["credits"].len(); i++) {
		if (creditsData["credits"][i]["type"] == "image") {
			creditsSprites.push(
				newSprite(
					creditsData["credits"][i]["image"],
					creditsData["credits"][i]["w"],
					creditsData["credits"][i]["h"],
					0,
					0,
					0,
					0
			...
			if (creditsData["credits"][i]["type"] == "header")
				creditsLength += 4;
		}
	}
...
gmCredits <- function () {
	...
	for (local i = 0; i < creditsData["credits"].len(); i += 1) {
		switch (creditsData["credits"][i]["type"]) {
			case "normal":
				local text = creditsData["credits"][i].rawin("trText")
					? gvLangObj["credits"][creditsData["credits"][i]["trText"]]
					: creditsData["credits"][i]["text"];
				drawText(
					font,
					screenW() / 2 - text.len() * 3,
					y + screenH() - creditsOffset,
					text
				);
				y += fontH;
				break;
			...
			case "image":
				drawSprite(
					creditsSprites[i],
					creditsData["credits"][i]["f"] +
						((getFrames() * creditsData["credits"][i]["s"]) %
							creditsData["credits"][i]["l"]),
					(screenW() - creditsData["credits"][i]["w"]) / 2,
					y + screenH() - creditsOffset
				);
	...
	if (
		creditsOffset >= screenH() + creditsLength + 10 ||
		getcon("pause", "press")
	) {
