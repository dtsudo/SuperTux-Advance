...
Konqi <- class extends Player {
	...
	an = {
		...
		run = [24, 25, 26, 27, 27, 28, 29, 30, 31, 31],
		dive = [14, 15],
		crouch = [14, 15],
		getUp = [15, 14],
	...
	function run() {
		...
		if ((!inWater(x, y) || stats.weapon == "earth") && resTime <= 0) {
			...
			switch (anim) {
				case "stand":
					...
					if (canMove && stats.stamina > 0 && getcon("spec2", "hold", true, playerNum) && !freeDown2) {
						anim = "ram";
						if (flip == 0) hspeed = max(hspeed, 3.0);
						else hspeed = min(hspeed, -3.0);
				...
				case "ram":
					...
					if (
						fabs(hspeed) < (!placeFree(x, y + 8) && placeFree(x + hspeed, y + 1) ? 7 : 5) &&
						stats.stamina > 0
					)
						hspeed *= 1.05;
					...
					if ((flip == 0 && hspeed < 0) || (flip == 1 && hspeed > 0)) anim = "skid";
					break;
			...
			if (onIce()) {
				...
				if (
					((!getcon("down", "hold", true, playerNum) || fabs(hspeed) < 0.05) &&
						!freeDown &&
						stats.weapon != "earth") ||
					(fabs(hspeed) < 0.05 && stats.weapon == "earth" && !getcon("shoot", "hold", true, playerNum)) ||
					(stats.weapon == "earth" &&
						!getcon("shoot", "hold", true, playerNum) &&
			...
			// Controls
			if (
				((!placeFree(x - hspeed, y + 2) && vspeed >= 0) ||
					!placeFree(x, y + 2) ||
					anim == "climb" ||
					onPlatform()) &&
				!onWall &&
			...
			if (!freeDown2 || onPlatform()) {
				if (anim == "slide") {
					...
					if (hspeed > 0) {
						if (
							!((mspeed > 2 && getcon("right", "hold", true, playerNum)) || anim == "ram") ||
							anim == "crawl" ||
							!canMove
						)
							hspeed -= friction;
					...
					if (hspeed < 0) {
						if (
							!((mspeed > 2 && getcon("left", "hold", true, playerNum)) || anim == "ram") ||
							anim == "crawl" ||
							!canMove
						)
							hspeed += friction;
			...
			if (
				canMove &&
				(anim == "jumpT" || anim == "jumpU" || anim == "fall") &&
			...
			if (canMove)
				switch (stats.weapon) {
					...
					case "earth":
						...
						if (
							((!getcon("shoot", "hold", true, playerNum) && !getcon("down", "hold", true, playerNum)) ||
								stats.energy == 0) &&
							anim == "statue"
						) {
							anim = "stand";
			...
			if (canMove)
				switch (stats.subitem) {
					...
					case "water":
						if (
							getcon("spec1", "hold", true, playerNum) &&
							anim != "slide" &&
							anim != "hurt" &&
							stats.energy >= 0.5 &&
							getFrames() % 8 == 0 &&
			...
			if (canMove) {
				...
				if (config.stickspeed) {
					...
					if (abs(joyY(j.index)) > abs(joyX(j.index)) && abs(joyY(j.index)) > js_max * 0.1)
						mspeed = (2.0 / float(js_max)) * abs(joyY(j.index));
				}
	...
	function draw() {
		if (!hidden) {
			...
			if (!config.showTF) sprite = mySprNormal;
			...
			// After image
			if (
				(zoomies > 0 || anim == "stomp" || (anim == "statue" && vspeed > 4) || anim == "ram") &&
				getFrames() % 2 == 0
			)
