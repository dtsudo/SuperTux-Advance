...
ExplodeN <- class extends WeaponEffect {
	...
	function run() {
		...
		if (gvPlayer) {
			if (owner != gvPlayer.id)
				if (
					floor(frame) <= 1 &&
					distance2(x, y, gvPlayer.x, gvPlayer.y) < 64
				) {
					if (x < gvPlayer.x && gvPlayer.hspeed < 8)
						gvPlayer.hspeed += 0.5 * (power / 2.0);
					if (x > gvPlayer.x && gvPlayer.hspeed > -8)
						gvPlayer.hspeed -= 0.5 * (power / 2.0);
					if (y >= gvPlayer.y && gvPlayer.vspeed > -8)
						gvPlayer.vspeed -= 0.8 * (power / 2.0);
				}
		}

		if (gvPlayer2) {
			if (owner != gvPlayer2.id)
				if (
					floor(frame) <= 1 &&
					distance2(x, y, gvPlayer2.x, gvPlayer2.y) < 64
				) {
					if (x < gvPlayer2.x && gvPlayer2.hspeed < 8)
						gvPlayer2.hspeed += 0.5 * (power / 2.0);
					if (x > gvPlayer2.x && gvPlayer2.hspeed > -8)
						gvPlayer2.hspeed -= 0.5 * (power / 2.0);
					if (y >= gvPlayer2.y && gvPlayer2.vspeed > -8)
						gvPlayer2.vspeed -= 0.8 * (power / 2.0);
				}
		}
	}
	...
	function run() {
		...
		if (gvPlayer) {
			if (owner != gvPlayer.id)
				if (
					floor(frame) <= 1 &&
					distance2(x, y, gvPlayer.x, gvPlayer.y) < 96
				) {
					if (x < gvPlayer.x && gvPlayer.hspeed < 8)
						gvPlayer.hspeed += 0.5 * (power / 2.0);
					if (x > gvPlayer.x && gvPlayer.hspeed > -8)
		...
		if (gvPlayer2) {
			if (owner != gvPlayer2.id)
				if (
					floor(frame) <= 1 &&
					distance2(x, y, gvPlayer2.x, gvPlayer2.y) < 96
				) {
					if (x < gvPlayer2.x && gvPlayer2.hspeed < 8)
						gvPlayer2.hspeed += 0.5 * (power / 2.0);
					if (x > gvPlayer2.x && gvPlayer2.hspeed > -8)
		...
		switch (element) {
			case "fire":
				if (getFrames() % 2 == 0)
					newActor(
						FlameTiny,
						x + lendirX(8, angle - 90) - 4 + randInt(8),
						y + lendirY(8, angle - 90) - 4 + randInt(8)
					);
				break;
			case "ice":
				if (getFrames() % 2 == 0)
		...
		for (local i = 0; i < 16; i++) {
			local d =
				actor[
					newActor(
						FlameTiny,
						x + lendirX(randInt(8) + 16, i * 22.5),
						y + lendirY(randInt(8) + 16, i * 22.5)
					)
				];
...
FuseSpark <- class extends WeaponEffect {
	...
	function run() {
		shape.setPos(x, y);
		if (getFrames() % 4 == 0)
			newActor(FlameTiny, x - 1 + randInt(3), y - 1 + randInt(3));
	}
...
FuseLine <- class extends PathCrawler {
	...
	function run() {
		...
		if (
			speed == 0 &&
			"WeaponEffect" in actor &&
			actor["WeaponEffect"].len() > 0
		)
		...
		if (gvPlayer2) {
			if (owner != gvPlayer2.id)
				if (
					...
					if (x < gvPlayer2.x && gvPlayer2.hspeed < 8)
						gvPlayer2.hspeed += 0.8;
					if (x > gvPlayer2.x && gvPlayer2.hspeed > -8)
						gvPlayer2.hspeed -= 0.8;
					if (y >= gvPlayer2.y && gvPlayer2.vspeed > -8)
						gvPlayer2.vspeed -= 1.5;
				}
		}
	}
...
ExplodeI2 <- class extends WeaponEffect {
	...
	function run() {
		...
		newActor(
			Glimmer,
			x + lendirX(r, randInt(360)),
			y + lendirY(r, randInt(360))
		);
	...
	function draw() {
		for (local i = 0; i < 8; i++) {
			drawSprite(
				...
				x + lendirX(r, angle + i * 45 + 22.5) - camx,
				y + lendirY(r, angle + i * 45 + 22.5) - camy,
				angle + i * 45 + 22.5,
				0,
				min(1, float(timeLimit - frame) / 10.0),
				min(1, float(timeLimit - frame) / 10.0)
			);
			drawLight(
				sprLightIce,
		...
		if (gvPlayer2) {
			if (owner != gvPlayer2.id)
				if (
					...
					if (x < gvPlayer2.x && gvPlayer2.hspeed < 8)
						gvPlayer2.hspeed += 0.5;
					if (x > gvPlayer2.x && gvPlayer2.hspeed > -8)
						gvPlayer2.hspeed -= 0.5;
					if (y >= gvPlayer2.y && gvPlayer2.vspeed > -8)
						gvPlayer2.vspeed -= 0.8;
				}
		}
...
Shockball <- class extends WeaponEffect {
	...
	function physics() {
		...
		foreach (i in actor) {
			if (
				i instanceof Enemy &&
				distance2(x, y, i.x, i.y) <= tdist &&
				i.health > 0 &&
				!("squish" in i && i.squish) &&
				!hitTest(shape, i.shape) &&
				!i.notarget
			) {
...
NutMine <- class extends NutBomb {
	...
	function run() {
		...
		if (
			checkActor(owner) &&
			getcon("jump", "press", false, actor[owner].playerNum) &&
			hitTest(bounceShape, actor[owner].shape)
		)
			deleteActor(id);
	}
	...
	function destructor() {
		base.destructor();
		if (checkActor(owner) && hitTest(bounceShape, actor[owner].shape))
			actor[owner].vspeed = -10;
	}
};
...
Airball <- class extends WeaponEffect {
	...
	function physics() {
		...
		if (inWater(x, y)) angle = pointAngle(0, 0, hspeed, vspeed) - 90;
		else {
			angle = 0;
			vspeed += gravity;
		}
	...
	function run() {
		...
		if (gvPlayer) {
			if (
				...
				if (x > gvPlayer.x && gvPlayer.hspeed < 8)
					gvPlayer.hspeed += 0.2 * (power / 2.0);
				if (x < gvPlayer.x && gvPlayer.hspeed > -8)
					gvPlayer.hspeed -= 0.2 * (power / 2.0);
				if (gvPlayer.vspeed > -8)
					gvPlayer.vspeed -= 0.8 * (power / 2.0);
			}
		}
		...
		if (gvPlayer2) {
			if (
				...
				if (x > gvPlayer2.x && gvPlayer2.hspeed < 8)
					gvPlayer2.hspeed += 0.2 * (power / 2.0);
				if (x < gvPlayer2.x && gvPlayer2.hspeed > -8)
					gvPlayer2.hspeed -= 0.2 * (power / 2.0);
				if (gvPlayer2.vspeed > -8)
					gvPlayer2.vspeed -= 0.8 * (power / 2.0);
			}
		}
	}
...
DragBubble <- class extends WeaponEffect {
	...
	function run() {
		if (checkActor(owner)) {
			hspeed += lendirX(
				0.1,
				pointAngle(x, y, actor[owner].x, actor[owner].y)
			);
			vspeed += lendirY(
				0.1,
				pointAngle(x, y, actor[owner].x, actor[owner].y)
			...
			if (gvPlayer && hitTest(shape, gvPlayer.shape)) {
				gvPlayer.hspeed += lendirX(
					0.1,
					pointAngle(x, y, actor[owner].x, actor[owner].y)
				);
				gvPlayer.vspeed += lendirY(
					0.1,
					pointAngle(x, y, actor[owner].x, actor[owner].y)
				);
			}
