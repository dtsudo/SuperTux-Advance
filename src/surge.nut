...
Surge <- class extends Player {
	...
	function physics() {
		...
		if (hspeed != 0) {
			...
			if (placeFree(x + hspeed, y)) {
				...
				if (
					fabs(hspeed) + fabs(ehspeed) >= 4 &&
					(anim == "ball" || !placeFree(x + hspeed, y)) &&
					y < yprev &&
					anim != "jumpR" &&
					anim != "jumpU" &&
					!nowInWater
	...
	function animation() {
		...
		switch (anim) {
			...
			case "charge":
				...
				if (chargeTimer < 10) {
					if (
						getcon("left", "press", true, playerNum) ||
	...
	function ruNormal() {
		...
		if (canMove) {
			...
			if (nowInWater && stats.weapon != "water") accel *= 0.6;
		...
		// Attacks
		if (!freeDown2 || nowInWater || anim == "wall") didAirSpecial = false;
		...
		if (
			stats.weapon == "ice" &&
			(!checkActor(homingTarget) ||
				hitTest(shape, actor[homingTarget].shape) ||
				anim != "jumpR" ||
				!placeFree(x + hspeed, y + vspeed))
		) {
			antigrav = 0;
			homingTarget = null;
			didAirSpecial = false;
	...
	function draw() {
		...
		if (anim == "ball" && stats.weapon == "earth")
			...
			drawSpriteZ(
				...
				sprite,
				an[anim][floor(frame)] + animOffset,
				x - camx + walkAngle / 20.0,
				y - 2 - camy + yoff + fabs(walkAngle / 15.0),
				walkAngle,
				flip,
				1,
				1,
			...
			switch (stats.weapon) {
				case "fire":
					drawSpriteZ(
						...
						1,
						1,
						fabs(0.5 - float(getFrames() % 60) / 60.0) * 2.0
					);
					break;
				case "ice":
					drawSpriteZ(
