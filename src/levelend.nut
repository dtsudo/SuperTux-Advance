...
LevelEnder <- class extends Actor {
	...
	function run() {
		if (((gvPlayer && gvPlayer.endMode) || !gvPlayer) && ((gvPlayer2 && gvPlayer2.endMode) || !gvPlayer2)) {
			{
				timer--;
...
endGoal <- function (speed = 0, unblock = "") {
	...
	if (levelEndRunner == 0) {
		...
		if (game.secrets >= game.maxSecrets && !game.allSecrets.rawin(clearedLevel))
			game.allSecrets[clearedLevel] <- true;
		if (game.enemies >= game.maxEnemies && !game.allEnemies.rawin(clearedLevel))
			game.allEnemies[clearedLevel] <- true;
		...
		if (!game.bestTime.rawin(clearedLevel + "-" + game.playerChar))
			...
			newActor(AchiNotice, 16, -16, format(gvLangObj["stats"]["new-pb"], formatTime(gvIGT), formatTime(game.bestTime[clearedLevel + "-" + game.playerChar])));
			game.bestTime[clearedLevel + "-" + game.playerChar] <- gvIGT;
		}
		...
		if (ghostRecordNew.len() < ghostRecordOld.len() || ghostRecordOld.len() <= 1) {
			if (gvNumPlayers == 1) {
				// Second method for storing two-player runs is in the works
				local timeString = "";
